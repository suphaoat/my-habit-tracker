...
from streamlit_calendar import calendar # <--- แก้ไขชื่อเครื่องมือที่ถูกต้องตรงนี้

# ตั้งชื่อไฟล์สำหรับเก็บข้อมูล
DATA_FILE = "habit_data.csv"
HABIT_FILE = "habits_list.txt"
NOTES_FILE = "notes_data.csv" 

# ---------------------------------------------------------
# 1. ระบบจัดการ "รายชื่อกิจกรรม"
# ---------------------------------------------------------
default_habits = [
...
# ---------------------------------------------------------
# 2. เมนูด้านข้าง (Sidebar) + นาฬิกา Real-time 
# ---------------------------------------------------------
with st.sidebar:
    st.title("🧭 เมนูหลัก")

    # --- โค้ดสร้างนาฬิกาธีมดำแบบแฮกเกอร์ ---
    clock_html = """
    <div id="clock" style="
...
# =========================================================
# หน้าที่ 1 : Habit Tracker
# =========================================================
if page == "✅ Habit Tracker":
    st.title("✅ My Daily Habit Tracker")
    st.write("บันทึกกิจกรรมประจำวันและดูสถิติย้อนหลัง")
    ...
    st.write("เลือกสถานะของแต่ละกิจกรรม (ระบบตั้งค่าเริ่มต้นที่ 'ไม่ได้ทำ')")
...
# =========================================================
# หน้าที่ 2 : ปฏิทินแพลนเนอร์รายเดือนขนาดใหญ่
# =========================================================
elif page == "📅 ปฏิทินแพลนเนอร์":
    st.title("📅 ปฏิทินแพลนเนอร์")
    st.write("จดแพลนงาน หรือนัดหมายต่างๆ (ข้อมูลแยกอิสระ ไม่นำไปรวมในกราฟสถิติ)")
    ...
    # 2. แปลงโน้ตเป็น "Events" สำหรับปฏิทิน
    calendar_events = []
    for index, row in df_notes.iterrows():
        calendar_events.append({
            ...
            "start": row["วันที่"], 
            "allDay": True, 
            "backgroundColor": "#00FFAA", # กรอบและพื้นหลังสีเขียวสะท้อนแสง
            "borderColor": "#00FFAA",
            "textColor": "#000000" # ตัวหนังสือสีดำให้เห็นชัดๆ
        })

    # 3. ตั้งค่าหน้าต่างปฏิทิน
    calendar_options = {
        "headerToolbar": {
